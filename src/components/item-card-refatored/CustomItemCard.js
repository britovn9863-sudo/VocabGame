...
export class CustomItemCard extends CustomContainer {
  ...
  async updateImage(newItem) {
    // Sempre que a atividade/item muda, limpa qualquer feedback visual
    // deixado pela atividade anterior.
    if (this.background) {
    ...
    const IMAGE_URL = `${import.meta.env.BASE_URL}assets/categories/${encodeURIComponent(
      this.category.title
  ...
  /**
   * Restaura o feedback visual do card para o estado normal da atividade.
   * Isso deve ser chamado sempre que uma nova palavra/rodada começar,
   * mesmo quando a mesma imagem voltar a aparecer depois.
   */
  resetFeedback() {
    if (this.background) {
      this.changeBorderColor(this.styles.borderColor);
    ...
    return new Promise((resolve) => {
      ...
      const shake = setInterval(() => {
        ...
        if (count > 8) {
          ...
          // Mantém a borda vermelha como feedback visual até a próxima atividade.
  ...
  showCorrectAnimation() {
    ...
    setTimeout(() => {
      this.y = originalY;
      // Mantém a borda verde como feedback visual até a próxima atividade.
